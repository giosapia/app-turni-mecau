...
if 'session_key' not in st.session_state or st.session_state.session_key != f"{anno}-{mese_idx}":
    ...
    st.session_key = f"{anno}-{mese_idx}"

tab1, tab2, tab3 = st.tabs(["📅 Desiderata & Jolly", "🛠️ Griglia Turni", "📊 Bilancio"])

with tab1:
    st.markdown("### ⚙️ Inserimento Jolly e Indisponibilità")
    # RIPRISTINO DEI BOX DI SELEZIONE (SelectboxColumn)
    config_jolly = {m: st.column_config.SelectboxColumn(m, options=["", "Ferie", "Corso", "Blocco", "No Giorno", "No Notte"], width="medium") for m in strutturati}
    st.session_state.df_desid = st.data_editor(st.session_state.df_desid, column_config=config_jolly, use_container_width=True)

with tab2:
    if st.button("🪄 Genera e Popola Bassa Intensità", type="primary"):
        ...
        for i in range(num_giorni):
            ...
            # 2. MECAU 1, 2 E BASSA INTENSITÀ
            for t in ["MeCAU 1", "MeCAU 2", "Bassa Intensità"]:
                # Ri-ordiniamo per bilanciare le ore ad ogni assegnazione
                cand_giorno = sorted(strutturati, key=lambda m: (df == m).sum().sum())
                for m in cand_giorno:
                    ...
                    if i > 0 and df.at[i-1, "MeCAU Notte"] == m: continue # Riposo post-notte
                    
                    df.at[i, t] = m; break
        st.session_state.df_turni = df; st.rerun()

    # Visualizzazione della griglia con Bassa Intensità abilitata
...
with tab3:
    ...
    for m in strutturati:
        ...
        bilancio.append({"Medico": m, "Ore Lavorate": lav, "Abbuono": abb, "Totale": lav+abb, "Delta": round(lav+abb-target_ore, 1)})
