...
def suggerisci_turni():
    ...
    for idx in range(len(df)):
        giorno_corrente = df.at[idx, "Giorno"]
        
        # 1. Assegnazione Notte, MeCAU 1 e MeCAU 2 (Strutturati)
        for col in ["MeCAU Notte", "MeCAU 1", "MeCAU 2"]:
            if not str(df.at[idx, col]).strip():
                ...
                for med in medici_shuffled:
                    pref = ds.at[giorno_corrente, med]
                    ...
                    # Controllo target ore
                    ore_tot = (df == med).sum().sum() * 12
                    abb = ds[med].isin(["Ferie", "Corso"]).sum() * 7.6
                    if (ore_tot + abb + 12) > (target_ore + 12): continue # Tolleranza di un turno
                    
                    df.at[idx, col] = med
                    break

        # 2. Assegnazione Bassa Intensità (Gettonisti)
        if not str(df.at[idx, "Bassa Intensità"]).strip() and gettonisti:
            df.at[idx, "Bassa Intensità"] = random.choice(gettonisti)
...
with tab1:
    st.info("Inserisci qui le assenze degli strutturati per bloccare la generazione automatica.")
...
with tab2:
    ...
    # LISTE PER IL MENU A TENDINA NELLA GRIGLIA
    lista_m_j = [""] + strutturati + jolly
    lista_b = [""] + gettonisti
    ...
    st.session_state.df_turni = st.data_editor(st.session_state.df_turni, column_config={
        ...
        "MeCAU 1": st.column_config.SelectboxColumn("MeCAU 1", options=lista_m_j),
        "MeCAU 2": st.column_config.SelectboxColumn("MeCAU 2", options=lista_m_j),
        "MeCAU Notte": st.column_config.SelectboxColumn("MeCAU Notte", options=lista_m_j),
        "Bassa Intensità": st.column_config.SelectboxColumn("Bassa Intensità", options=lista_b),
    }, use_container_width=True, hide_index=True)

with tab3:
    st.subheader("📊 Riepilogo Bilancio Ore (Strutturati)")
    report = []
    for m in strutturati:
        ore_l = (st.session_state.df_turni.iloc[:, 1:4] == m).sum().sum() * 12
        abb = st.session_state.df_desid[m].isin(["Ferie", "Corso"]).sum() * 7.6
        diff = round((ore_l + abb) - target_ore, 1)
        report.append({"Medico": m, "Ore Lavorate": ore_l, "Abbuono (Ferie/Corso)": abb, "Totale": round(ore_l + abb, 1), "Target": round(target_ore, 1), "Delta": diff})
