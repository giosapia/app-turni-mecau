...
def get_festivi(year):
    ...
    # Calcolo Pasqua e Lunedì dell'Angelo
...
def crea_pdf_fpdf(df, anno, mese_nome):
    ...
    for _, row in df.iterrows():
        fill = False
        label = str(row['Giorno'])
        if label.startswith("F "):
            pdf.set_fill_color(255, 200, 200)
            fill, label = True, label.replace("F ", "")
        elif label.startswith("S "):
            pdf.set_fill_color(255, 255, 180)
            fill, label = True, label.replace("S ", "")
        pdf.cell(widths[0], 8, label, border=1, fill=fill)
        for col in cols[1:]:
...
st.sidebar.metric("Monte Ore Target (Feriali)", f"{target_ore:.1f}h")
        ...
        for col in ["MeCAU Notte", "MeCAU 1", "MeCAU 2"]:
            if not str(df.at[idx, col]).strip():
                ...
                for med in medici_shuffled:
                    # CONTEGGIO ORE TOTALE (Feriali + Festivi + Notti)
                    ore_totali_pianificate = (df == med).sum().sum() * 12
                    ferie_giorni = ds[med].isin(["Ferie", "Corso", "Blocco"]).sum()
                    abbuono = ferie_giorni * 7.6
                    
                    # Se il prossimo turno (12h) sfora il target, non assegnare
                    if (ore_totali_pianificate + abbuono + 12) > target_ore:
                        continue
                    
                    # Vincoli di riposo e 48h
...
with tab2:
    ...
    with c2:
        if st.button("Scarica PDF"):
            pdf_data = crea_pdf_fpdf(st.session_state.df_turni, anno, calendar.month_name[mese_idx])
...
with tab3:
    st.subheader("📊 Bilancio Ore (Feriali + Festivi)")
    report = []
    for m in strutturati:
        ...
        report.append({"Medico": m, "Ore Lavorate (Tot)": ore_l, "Abbuono": abb, "Target": round(target_ore, 1), "Differenza": round((ore_l + abb) - target_ore, 1)})
